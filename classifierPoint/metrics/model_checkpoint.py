...
class Checkpoint:
    ...
    @staticmethod
    def load(checkpoint_file: str, run_config: Any, resume=True, mode="train"):
        ...
        """
        # when checkpoint_dir=""  local path have model
        if not os.path.exists(checkpoint_file):
            if mode == "eval":
                message = "The provided path {} didn't contain the checkpoint_file".format(checkpoint_file)
            ...
            if resume:
                # for external load
                if not os.path.samefile(os.getcwd(), os.path.dirname(checkpoint_file)):
                    shutil.copyfile(
    ...
    def load_optim_sched(self, model, load_state=True):
        if not self.is_empty:
            ...
            optimizer_params = OmegaConf.create(self.run_config).training.optimizer.params
            optimizer_params.lr = self.run_config["training"]["lr"]
...
class ModelCheckpoint(object):
    ...
    def __init__(
        ...
        self._checkpoint = Checkpoint.load(load_dir, run_config=rc, resume=resume, mode=mode)
        self._resume = resume
        self._selection_stage = selection_stage
        self._data_config = OmegaConf.create(self._checkpoint.run_config).data
