...
class DynamicLinearBlock(RandomModule):
    def __init__(self, inc, outc, cr_bounds=(0.25, 1.0), bias=True, no_relu=False, no_bn=False):
        super().__init__()
        self.inc = inc
        self.outc = outc
        self.bias = bias
    ...
    def re_organize_middle_weights(self):
        ...
        self.net.conv.linear.weight.data = torch.index_select(self.net.conv.linear.weight.data, 0, sorted_idx)
        if self.bias:
            self.net.conv.linear.bias.data = torch.index_select(self.net.conv.linear.bias.data, 0, sorted_idx)
    ...
    def determinize(self):
        ...
        inc = self.runtime_inc if self.runtime_inc is not None else len(self.in_channel_constraint)

        determinized_model = LinearBlock(
            inc,
            self.runtime_outc,
            bias=self.bias,
...
class ConvolutionBlock(nn.Module):
    def __init__(self, inc, outc, ks=3, stride=1, dilation=1, no_relu=False, transposed=False):
        super().__init__()
        self.inc = inc
        self.outc = outc
        self.ks = ks
        ...
        self.net = nn.Sequential(
            OrderedDict(
                [
                    ...
                    ("act", spnn.ReLU(True) if not self.no_relu else nn.Sequential()),
                ]
            )
        )
        self.init_weights()
...
class DynamicConvolutionBlock(RandomModule):
        ...
        self.net = nn.Sequential(
            OrderedDict(
                [
                    (
                        "conv",
                        SparseDynamicConv3d(inc, outc, kernel_size=ks, dilation=dilation, stride=stride),
                    ),
    ...
    def re_organize_middle_weights(self):
        ...
        sorted_importance, sorted_idx = torch.sort(importance, dim=0, descending=True)
        if len(weights.shape) == 3:
            self.net.conv.kernel.data = torch.index_select(self.net.conv.kernel.data, 2, sorted_idx)
        else:
            self.net.conv.kernel.data = torch.index_select(self.net.conv.kernel.data, 1, sorted_idx)
        adjust_bn_according_to_idx(self.net.bn.bn, sorted_idx)
    ...
    def determinize(self):
        ...
        determinized_model = ConvolutionBlock(inc, self.runtime_outc, self.ks, self.s, no_relu=self.no_relu)
        determinized_model.load_weights(self, self.in_channel_constraint)
        return determinized_model
...
class DynamicDeconvolutionBlock(RandomModule):
        ...
        self.net = nn.Sequential(
            OrderedDict(
                [
                    (
                        "conv",
                        SparseDynamicConv3d(inc, outc, kernel_size=ks, stride=stride, transposed=True),
                    ),
                    ("bn", SparseDynamicBatchNorm(outc)),
                    ("act", spnn.ReLU(True)),
                ]
    ...
    def determinize(self):
        determinized_model = ConvolutionBlock(self.runtime_inc, self.runtime_outc, self.ks, self.s, transposed=True)
        determinized_model.load_weights(self, self.in_channel_constraint)
        return determinized_model
...
class DynamicResidualBlock(nn.Module):
    def __init__(self, inc, outc, cr_bounds=(0.25, 1.0), ks=3, stride=1, dilation=1):
        ...
        self.net = RandomDepth(
            *[
                DynamicConvolutionBlock(inc, outc, cr_bounds, ks, stride, dilation, False),
                DynamicConvolutionBlock(outc, outc, cr_bounds, ks, stride, dilation, True),
            ],
            depth_min=2,
        )
        ...
        if self.use_skip_conn:
            ...
            self.downsample = DynamicConvolutionBlock(inc, outc, cr_bounds, ks=1, stride=1, dilation=1, no_relu=True)
    ...
    def determinize(self):
        ...
        downsample = nn.Sequential() if not self.use_skip_conn else self.downsample.determinize()
