...
from classifierPoint.core.schedulers.lr_schedulers import instantiate_scheduler
from classifierPoint.core.schedulers.bn_schedulers import instantiate_bn_scheduler
from classifierPoint.utils.enums import SchedulerUpdateOn
from classifierPoint.config.IGNORE_LABEL import IGNORE_LABEL

from classifierPoint.core.regularizer import *
from classifierPoint.core.losses import instantiate_loss_or_miner
...
class BaseModel(torch.nn.Module, TrackerInterface, DatasetInterface, CheckpointInterface):
    ...
    def set_pretrained_weights(self):
        path_pretrained = getattr(self.opt, "path_pretrained", "")
        weight_name = getattr(self.opt, "weight_name", "best_miou")
    ...
    def optimize_parameters(self, epoch, batch_size):
        ...
        self.compute_loss()  # calculate losses
    ...
    def instantiate_optimizers(self, config, cuda_enabled=False):
        ...
        # LR Scheduler
        scheduler_opt = self.get_from_opt(config, ["training", "lr_scheduler"])
        if scheduler_opt:
        ...
        # BN Scheduler
        bn_scheduler_opt = self.get_from_opt(config, ["training", "bn_scheduler"])
        if bn_scheduler_opt:
        ...
        self._accumulated_gradient_step = self.get_from_opt(config, ["training", "accumulated_gradient"])
        ...
        # Gradient clipping
        self._grad_clip = self.get_from_opt(config, ["training", "grad_clip"], default_value=-1)
    ...
    def get_metric_loss_and_miner(opt_loss, opt_miner):
        ...
        if opt_loss is not None:
            if not getattr(opt_loss, "params", None):
                opt_loss["params"] = {}
            if "ignore_index" not in opt_loss["params"].keys():
                opt_loss["params"].update({"ignore_index": IGNORE_LABEL})
    ...
    def log_optimizers(self):
        log.info("Optimizer: {}".format(self._optimizer))
        log.info("Learning Rate Scheduler: {}".format(self._lr_scheduler))
        log.info("BatchNorm Scheduler: {}".format(self._bn_scheduler))
        log.info("Accumulated gradients: {}".format(self._accumulated_gradient_step))
