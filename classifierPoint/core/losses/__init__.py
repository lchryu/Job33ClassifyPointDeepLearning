...
_custom_losses = sys.modules["classifierPoint.core.losses.losses"]
_origin_losse = sys.modules["torch.nn"]
...
def instantiate_loss_or_miner(option, mode="loss"):
    ...
    if "loss" in mode:
        ...
        if not cls:
            cls = getattr(_origin_losse, class_, None)
            if not cls:
                raise ValueError("loss {} is nowhere to be found".format(class_))
    else:
