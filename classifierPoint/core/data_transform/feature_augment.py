...
class ChromaticAutoContrast(object):
    ...
    def __call__(self, data):
        ...
        if random.random() < 0.2:
            feats = data.rgb
            low = feats.min(0, keepdims=True)[0]
            high = feats.max(0, keepdims=True)[0]
            assert high.max() > 0, "invalid color value. Color is supposed to be [0-255]"

            scale = 1.0 / (high - low)

            contrast_feats = (feats - low) * scale
...
class Rescale:
    ...
    def __init__(self, feat_name, randomize_blend_factor=True, blend_factor=0.5):
        self.randomize_blend_factor = randomize_blend_factor
        self.blend_factor = blend_factor
        self.feat_name = feat_name

    def __call__(self, data):
        assert hasattr(data, self.feat_name)
        if random.random() < 0.2:
            feats = getattr(data, self.feat_name, None)
            low = feats.min(0, keepdims=True)[0]
            high = feats.max(0, keepdims=True)[0]
            scale = 1.0 / (high - low)
            contrast_feats = (feats - low) * scale
            ...
            blend_factor = random.random() if self.randomize_blend_factor else self.blend_factor
            new_feat = (1 - blend_factor) * feats + blend_factor * contrast_feats
            setattr(data, self.feat_name, new_feat)
        return data

    def __repr__(self):
        return "{}(feat_name={},randomize_blend_factor={}, blend_factor={})".format(
            self.__class__.__name__, self.feat_name, self.randomize_blend_factor, self.blend_factor
        )
...
class YRescale:
    ...
    def __call__(self, data):
        if random.random() < 0.2:
            feats = data.coords
            low = feats.min(0, keepdims=True).values[0][2]
            high = feats.max(0, keepdims=True).values[0][2]
            scale = 1.0 / (high - low)
            contrast_feats = (feats[:, 2] - low) * scale

            blend_factor = random.random() if self.randomize_blend_factor else self.blend_factor
            new_feat = (1 - blend_factor) * feats[:, 2] + blend_factor * contrast_feats
            feats[:, 2] = new_feat
            data.coords = feats
        return data
