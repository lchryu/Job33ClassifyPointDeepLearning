...
from classifierPoint.datasets.multiscale_data import MultiScaleData
from classifierPoint.utils.transform_utils import SamplingStrategy
from classifierPoint.utils.config import is_list
from classifierPoint.utils import is_iterable
from .grid_transform import group_data, VoxelGrid, shuffle_data
from .features import RandomRotation
...
class GridSphereSampling(object):
    ...
    def __init__(self, radius, grid_size=None, delattr_kd_tree=True, center=True):
        ...
        self._grid_sampling = VoxelGrid(size=grid_size if grid_size else self._radius)
    ...
    def __call__(self, data):
        if self._apply_shift:
            if not hasattr(data, "coords"):
                raise Exception("should quantize first using VoxelGrid")
...
class RandomDropout:
    ...
    def __init__(self, dropout_ratio: float = 0.2, dropout_application_ratio: float = 0.5, class_id=None):
    ...
    def __call__(self, data):
        if random.random() < self.dropout_application_ratio:
            if self.class_id:
                ...
                mask = [torch.where(data.y == i)[0] for i in self.class_id]
                mask = torch.concat(mask)
                N = len(mask)
                choice = np.random.choice(N, int(N * (1 - self.dropout_ratio)), replace=True)
                choice = torch.from_numpy(choice).to(torch.long)

                all_classid = list(range(-1,data.y.max() + 1))
                reverse_classid=[x for x in all_classid if x not in self.class_id]
                reverse_mask = [torch.where(data.y == i)[0] for i in reverse_classid]
                reverse_mask.append(choice)  # reverse_mask+choice
                finally_mask = torch.concat(reverse_mask)
                data = apply_mask(data, finally_mask)
            else:
                N = len(data.pos)
...
class RandomSphereDropout(object):
    ...
    def __init__(self, num_sphere: int = 10, radius: float = 5, grid_size_center: float = 0.01):
        ...
        self.radius = radius
        self.grid_sampling = VoxelGrid(grid_size_center, mode="last")
...
class CubeCrop(object):
    ...
    def __init__(
        ...
        self.c = c
        self.random_rotation = RandomRotation(rot_x=rot_x, rot_y=rot_y, rot_z=rot_z)
    def _norm(self,data):
        #按照长轴归一化
    ...
    def __call__(self, data):
        ...
        data = apply_mask(data, mask)
        #data.xyz = self._norm(data.xyz)[:,2]
        return data
