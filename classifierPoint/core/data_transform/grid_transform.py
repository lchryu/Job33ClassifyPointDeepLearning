...
_INTEGER_LABEL_KEYS = ["y", "instance_labels", "origin_id"]
...
def group_data(data: Data, cluster=None, unique_pos_indices=None, mode="last", skip_keys=["grid_size"]):
...
class VoxelGrid:
    ...
    def __init__(self, size, quantize_coords=True, mean=True, verbose=False):
        self._grid_size = size
        self._quantize_coords = quantize_coords
        self._mode = "mean" if mean else "last"
        if verbose:
            log.warning(
                "If you need to keep track of the position of your points, use SaveOriginalPosId transform before using VoxelGrid"
            )
    ...
    def _process(self, data):
        # 这里在推理模式时将引起数据还原错误,原因:lidata的pos不会被打乱 label被打乱
        # if self._mode == "last":
        #     data = shuffle_data(data)
        #coords = torch.round((data.pos) / 0.05)
        coords = torch.round((data.pos) / self._grid_size)
        if "batch" not in data:
            cluster = grid_cluster(coords, torch.tensor([1, 1, 1]))
        else:
            cluster = voxel_grid(coords, batch=data.batch, size=1)
        cluster, unique_pos_indices = consecutive_cluster(cluster)
        data = group_data(data, cluster, unique_pos_indices, mode=self._mode)
        data.origin_id = cluster
        if self._quantize_coords:
            data.coords = coords[unique_pos_indices].type(torch.int)
