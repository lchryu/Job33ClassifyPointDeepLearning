...
from classifierPoint.core.data_transform import (
    instantiate_transforms,
    MultiScaleTransform,
)
from classifierPoint.core.data_transform import instantiate_filters
from classifierPoint.datasets.batch import SimpleBatch, HackTGBatch
from classifierPoint.datasets.multiscale_data import MultiScaleBatch
from classifierPoint.utils.enums import ConvolutionFormat
from classifierPoint.utils.config import ConvolutionFormatFactory
...
class BaseDataset:
    ...
    def feature_dimension(self):
        if self.train_dataset:
            feature_num = self.train_dataset.num_features
            if feature_num == 0:
                raise AttributeError(f" object has no feature'")
            return feature_num
        elif self.test_dataset is not None:
            if isinstance(self.test_dataset, list):
                feature_num = self.test_dataset[0].num_features
                if feature_num == 0:
                    raise AttributeError(f" object has no feature'")
                return feature_num
            else:
                feature_num = self.test_dataset.num_features
                if feature_num == 0:
                    raise AttributeError(f" object has no feature'")
                return feature_num
        elif self.val_dataset is not None:
            feature_num = self.val_dataset.num_features
            if feature_num == 0:
                raise AttributeError(f" object has no feature'")
            return feature_num
        else:
    ...
    def resolve_saving_stage(self, selection_stage):
        ...
        """
        log.info("Available stage selection datasets:  {} ".format(self.available_stage_names))
    ...
    def __repr__(self):
        ...
        for key, attr in self.__dict__.items():
            if key.endswith("_sampler") and attr:
                message += "{} = {}\n".format(key, attr)
        message += "Batch size ={}".format(self.batch_size)
        return message
